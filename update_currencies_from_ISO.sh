...
for cur in ${sorted[*]}; do
  if [[ ${cur_his[$cur]} == 1 ]]; then
    sed -i "/INSERT INTO CURRENCYFORMATS_V1 VALUES($max_id,/a INSERT INTO CURRENCYFORMATS_V1 VALUES($((++max_id)),'_tr_${cur_nam[$cur]}','','','.',',',100,1,'$cur','Fiat',1);" "$FILE" \
      && unset cur_nam[$cur] cur_his[$cur]
  else
    sed -i "/INSERT INTO CURRENCYFORMATS_V1 VALUES($max_id,/a INSERT INTO CURRENCYFORMATS_V1 VALUES($((++max_id)),'_tr_${cur_nam[$cur]}','','','.',',',$((10**${cur_dig[$cur]})),1,'$cur','Fiat',0);" "$FILE" \
      && unset cur_nam[$cur] cur_dig[$cur] cur_his[$cur]
  fi
done
